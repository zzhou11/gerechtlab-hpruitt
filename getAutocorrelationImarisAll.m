% Performs getAutocorrelationImaris on a file for all tracks, returns as a
...
fprintf('Running autocorrelation... \n');
...
for i = 1:size(usableTracks, 1)
    results = [results; usableTracks(i, 1), getAutocorrelationImaris(newData, usableTracks(i, 1))]; 
end

header = {'Track ID', 'Autocorrelation'};
cd(destination);
output = [header; num2cell(results)];
writecell(output, strcat(file, ' autocorrelation results.xlsx'));
cd(oldFolder);
